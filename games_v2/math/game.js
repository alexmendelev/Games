(function () {
  ...
  const cfg = window.GAME_V2_MATH_CONFIG;
  ...
  function makeTask() {
    ...
    const op = utils.choice(diff.ops);
    ...
    if (op === "+") {
      a = utils.randInt(0, 10);
      b = utils.randInt(0, 10);
      answer = a + b;
      text = `${a}+${b}`;
    } else if (op === "-") {
      a = utils.randInt(0, 10);
      b = utils.randInt(0, 10);
      if (!diff.allowNegResult && b > a) {
        [a, b] = [b, a];
      }
      answer = a - b;
      text = `${a}-${b}`;
      ...
      a = utils.randInt(mulMin, 10);
      b = utils.randInt(mulMin, 10);
      answer = a * b;
      text = `${a}×${b}`;
    } else {
      b = utils.randInt(1, 10);
      answer = utils.randInt(0, 10);
      a = answer * b;
      text = `${a}÷${b}`;
    }
  ...
  function uniqueWrongs(correct) {
    ...
    while (set.size < 4 && guard < 200) {
      ...
      if (Math.random() < 0.25) {
        wrong = correct + utils.randInt(-15, 15);
      }
      if (!ok(wrong)) {
        continue;
      }
